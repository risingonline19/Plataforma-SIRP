...
openssl req -days 3650 -new -key admin-key.pem -out admin.csr -subj "/C=ES/L=Barcelona/O=CyberHome Ltd/CN=admin"
...
openssl req -days 3650 -new -key node-key.pem -out node.csr -subj "/C=ES/L=Barcelona/O=CyberHome Ltd/CN=*.wazuh-elasticsearch"
...
openssl req -days 3650 -new -key kibana-key.pem -out kibana.csr -subj "/C=ES/L=Barcelona/O=CyberHome Ltd/CN=kibana"
...
openssl req -days 3650 -new -key filebeat-key.pem -out filebeat.csr -subj "/C=ES/L=Barcelona/O=CyberHome/CN=filebeat.cyberhome.es"
